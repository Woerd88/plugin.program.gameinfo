...
Contributions:
24 april 2013   (Garbear)   - Added BigEndian type support + Pathtable
15 may   2015   (Woerd88)   - Added Directory Record + Additional Pathtables + ISO modes
...
class ISO9660(Parser):
    ...
    def createFields(self):
        ...
        # and add the sectorheader of 24 bytes
            ...
            self.sector_form = 1
            self.sector_size = 2352
            self.sector_header_size = 24

        #Step 2: Skip this system area, not part of the ISO9660 specifications
        yield self.seekByte( (16 * self.sector_size), null=True)
        ...
        while True:
            ...
            if volume["type"].value == 1: # PrimaryVolumeDescriptor
                ...
                pathtable_l_offset = volume["content/occu_lpath"].value * self.sector_size
                pathtable_l_size = volume["content/path_table_size_l"].value
                pathtable_m_offset = volume["content/occu_mpath"].value * self.sector_size
                pathtable_m_size = volume["content/path_table_size_m"].value

                # Read the optional ocurrences as well
                pathtable_opt_l_offset = volume["content/opt_lpath"].value * self.sector_size
                pathtable_opt_m_offset = volume["content/opt_mpath"].value * self.sector_size
        ...
        for i in range(0, 4):
            ...
            if pathtable_offset:
                ...
                while True:
                    ...
                    if currentpath >= pathtable_offset + pathtable_size + self.sector_header_size:
                        break

                #seek to the end of the sextor so we can mark the sector ending properly
                yield self.seekByte(pathtable_offset + self.sector_header_size + 2048, relative=False, null=True)
        ...
        for i in range(0, self._field_array_count["path_l"] + 1):
            ...
            if pathtable_entry["location"].value * self.sector_size * 8 >= self.current_size:

                directory_offset = pathtable_entry["location"].value * self.sector_size
                directory_size = 0
                ...
                while (directory_sector_total == 0 or  directory_sector_curr < directory_sector_total ):
                    ...
                    directory_offset = directory_offset + self.sector_header_size + 2048
